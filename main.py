...
# define mpos file path. User defined or the same directory as daf.
    writempos=True
    if (args.mpos=="default_directory"):
        directory = daf_file_path[:daf_file_path.rfind(".")+1]
        mpos_file_path=directory+"mpos"
    elif (args.mpos=="nompos"):
        writempos=False
        print("Script runs without generate the mpos file")
    else:
...
# define Time offset
    ...
    if (args.timeoffset!=None):
        ...
        print ("Timeoffset relative to system determined optical timeoffset was set to",  str(manual_timeoffset), "micro-sec")
# write lmdout file.
    if (args.mbr!=None):
